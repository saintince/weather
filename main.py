...
def register(user: UserCreate, db: Session = Depends(get_db)):
    ...
    user = User(username=user.username, hashed_password=hash_password(user.password))
    db.add(user)
    db.commit()
    db.refresh(user)
...
def get_favorites(db: Session = Depends(get_db), current_user: str = Depends(get_current_user)):
    user = db.query(User).filter(User.username == current_user).first()
    return db.query(FavCity).filter(FavCity.user_id == user.id).all()
...
def delete_city(city: str, db: Session = Depends(get_db), current_user: str = Depends(get_current_user)):
    user = db.query(User).filter(User.username == current_user).first()
    db_city = db.query(FavCity).filter(FavCity.city == city, FavCity.user_id == user.id).first()
    if not db_city:
